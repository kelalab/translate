...
När du startar applikationen möts du av tre huvudflikar: **Text**, **Dokument (Documents)** och **Bilder (Images)**.
...
4. **Historikfliken (History Drawer)**: Utöka "Översättningshistorik" längst ner i appen för att granska dina 20 senaste textöversättningar.
...
## 5. Avancerade Funktioner: Externa API-anslutningar
...
`npm start -- --enable-external-api` (eller `--advanced-api`)
